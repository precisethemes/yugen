...
class Yugen_Breadcrumb {
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= sprintf(
				'<%s class="trail-items d-flex flex-wrap align-items-center p-0 mb-0 ml-0 ls-none" itemscope itemtype="http://schema.org/BreadcrumbList">',
			...
			foreach ( $this->items as $item ) {
				...
				$item = ! empty( $matches ) ? sprintf( '%s<span itemprop="name">%s</span>%s%s', $matches[1], $matches[2], $del_symbol, $matches[3] ) : sprintf( '<span itemprop="name">%s</span>', $item );
			...
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="yugen breadcrumbs" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
