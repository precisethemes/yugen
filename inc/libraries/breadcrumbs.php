<?php
/** Code Adapted from Breadcrumb Trail
 ...
 */
/**
 * Shows a breadcrumb for all types of pages.  This is a wrapper function for the Yugen_Breadcrumb class,
 * which should be used in theme templates.
 *
 * @since  1.0.0
 * @access public
 * @param  array $args Arguments to pass to Yugen_Breadcrumb.
 * @return void
 */
function yugen_breadcrumb( $args = array() ) {

    $breadcrumb = apply_filters( 'yugen_breadcrumb_object', null, $args );

    if ( ! is_object( $breadcrumb ) )
        $breadcrumb = new Yugen_Breadcrumb( $args );
...
class Yugen_Breadcrumb {
    ...
    /**
     * Sets up the breadcrumb trail properties.  Calls the `Yugen_Breadcrumb::add_items()` method
     ...
     *     @type array     $labels         Text labels. @see Yugen_Breadcrumb::set_labels()
     *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Yugen_Breadcrumb::set_post_taxonomy()
    ...
    public function __construct( $args = array() ) {
        ...
        $this->args = apply_filters( 'yugen_breadcrumb_args', wp_parse_args( $args, $defaults ) );
    ...
    public function trail() {
        ...
        if ( 0 < $item_count ) {
            ...
            $breadcrumb .= sprintf(
                '<%s class="trail-items d-flex flex-wrap align-items-center p-0 mb-0 ml-0 ls-none">',
            ...
            foreach ( $this->items as $item ) {
                ...
                $del_symbol = ! empty( $delimiter ) ? sprintf( '<span class="delimiter">%s</span>', $delimiter ) : '';

                // Wrap the item text.
                $item = ! empty( $matches ) ? sprintf( '%s%s%s%s', $matches[1], $matches[2], $del_symbol, $matches[3] ) : sprintf( '%s', $item );

                // Wrap the item.
                $item = ! empty( $matches )
                    ? preg_replace( '/(<a.*?)([\'"])>/i', '$1$2 rel="v:url" property="v:title">', $item )
                    : sprintf( '%s', $item );
                ...
                // Build the list item.
                $breadcrumb .= sprintf( '<%1$s %2$s typeof="v:Breadcrumb">%3$s%4$s</%1$s>', tag_escape( $this->args['item_tag'] ),$attributes, $item, $meta );
            }
            ...
            $breadcrumb = sprintf(
                '<%1$s role="navigation" aria-label="%2$s" class="yugen-breadcrumbs breadcrumbs">%3$s%4$s%5$s</%1$s>',
        ...
        $breadcrumb = apply_filters( 'yugen_breadcrumb', $breadcrumb, $this->args );
    ...
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'yugen' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'yugen' ),
            'home'                => esc_html__( 'Home',                                  'yugen' ),
            'error_404'           => esc_html__( '404 Not Found',                         'yugen' ),
            'archives'            => esc_html__( 'Archives',                              'yugen' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'yugen' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'yugen' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'yugen' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'yugen' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'yugen' ),
        ...
        $this->labels = apply_filters( 'yugen_breadcrumb_labels', wp_parse_args( $this->args['labels'], $defaults ) );
    }
    ...
    protected function set_post_taxonomy() {
        ...
        $this->post_taxonomy = apply_filters( 'yugen_breadcrumb_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
    }
    ...
    protected function add_items() {
        ...
        $this->items = array_unique( apply_filters( 'yugen_breadcrumb_items', $this->items, $this->args ) );
    }
    ...
    protected function add_network_home_link() {
        ...
        if ( is_multisite() && ! is_main_site() && true === $this->args['network'] )
            $this->items[] = sprintf( '<a href="%s" rel="v:url" property="v:title">%s</a>', esc_url( network_home_url() ), $this->labels['home'] );
    }
    ...
    protected function add_site_home_link() {
        ...
        $rel     = $network ? '' : ' rel="v:url" property="v:title"';
    ...
    protected function add_minute_hour_archive_items() {
        ...
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'yugen' ) ) );
    }
    ...
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'yugen' ) ) );
    }
    ...
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
            $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'yugen' ) ) );
    }
    ...
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'yugen' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'yugen' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'yugen' ) ) );
    ...
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'yugen' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'yugen' ) ) );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'yugen' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'yugen' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag )
                    $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'yugen' ) ) ) );
